...
pool_v3
  ...
  .onEventSwapEventV3(async (evt, ctx) => {
    ...
    if (infoX.symbol.includes('USD') && infoY.symbol.includes('USD')) {
      recordSwap(ctx, ctx.transaction.sender, infoX.symbol, infoY.symbol, 'hyperion')
    }
  })
...
pool_v3.bind().onEventCreatePoolEvent(async (evt, ctx) => {
  ...
  console.log('new hyperion pool', pool)
  let _ctx = ctx as any
  _ctx.timestampInMicros = new Date(ctx.transaction.timestamp).valueOf()
  if (infoA.symbol.includes('USD')) {
    template.bind({ address: storeA, startVersion: DEFI_START_VERSION }, _ctx)
  }
  if (infoB.symbol.includes('USD')) {
    template.bind({ address: storeB, startVersion: DEFI_START_VERSION }, _ctx)
  }
})
