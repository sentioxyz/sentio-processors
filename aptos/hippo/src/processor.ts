...
import { getPriceForToken, getTokenInfoWithFallback, isWhiteListToken } from "@sentio/sdk/aptos/ext"
...
aggregator.bind({ address: "0x89576037b3cc0b89645ea393a47787bb348272c76d6941c574b053672b848039" })
  .onEventSwapStepEvent(async (evt, ctx) => {
    ...
    const coinXInfo = await getTokenInfoWithFallback(xType)
    const coinYInfo = await getTokenInfoWithFallback(yType)
    const poolType = evt.data_decoded.pool_type
    const priceX = await getPriceForToken(xType, Number(timestamp))
    const priceY = await getPriceForToken(yType, Number(timestamp))
