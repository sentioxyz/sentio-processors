...
async function getERC721Name(nftAddress: string, hash_debug: string) {
  ...
  if (!collectionName) {
    try {
      collectionName = await getERC721Contract(EthChainId.ETHEREUM, nftAddress).name()!
...
async function getERC1155Name(nftAddress: string, id: number, hash_debug: string) {
  ...
  if (!collectionName) {
    try {
      const collectionName = await getERC721Contract(EthChainId.ETHEREUM, nftAddress).name()!
      nftCollectionMap.set(nftAddress, collectionName)
