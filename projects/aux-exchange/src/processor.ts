...
import { AptosDex, getTokenInfoWithFallback } from "@sentio/sdk/aptos/ext"
...
amm.bind({ startVersion: 2331560 })
  ...
  .onEventRemoveLiquidityEvent(async (evt, ctx) => {
    ...
    ctx.eventLogger.emit("User", { distinctId: ctx.transaction.sender })
    const coinXInfo = await getTokenInfoWithFallback(evt.data_decoded.x_coin_type)
    const coinYInfo = await getTokenInfoWithFallback(evt.data_decoded.y_coin_type)
  ...
  .onEventSwapEvent(async (evt, ctx) => {
    ...
    //
    const coinXInfo = await getTokenInfoWithFallback(evt.data_decoded.in_coin_type)
    const coinYInfo = await getTokenInfoWithFallback(evt.data_decoded.out_coin_type)
