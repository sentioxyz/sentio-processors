...
managers.forEach((address) => {
  CurvanceMarketManagerProcessor.bind({ network, address, startBlock: START_BLOCK }).onTimeInterval(
    async (block, ctx) => {
      const { tokenX, tokenY } = await getPoolInfo(ctx, ctx.address)
      await Promise.all([recordCurvanceTvl(ctx, tokenX), recordCurvanceTvl(ctx, tokenX)])
    },
    60 * 12,
    60 * 24,
  )
...
async function recordCurvanceTvl(ctx: EthContext, token: string) {
  ...
  await recordTvl(ctx, asset, totalAssets, project)
}
