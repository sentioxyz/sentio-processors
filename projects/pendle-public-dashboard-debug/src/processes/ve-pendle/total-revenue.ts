...
async function handleSYTransfer(
  event: TransferEvent,
  ctx: StandardizedYieldContext
) {
  try {
    ...
    const feeType = (await senderContract.symbol()).startsWith("YT")
      ? FEE_TYPE.YT_FEE
      : FEE_TYPE.SWAP_FEE;

    const symbol = await getSYName(ctx);
    revenueByToken.add(ctx, amount.times(price), {
      token: symbol,
      type: feeType,
    });
...
async function handleRewardTokenTransfer(
  event: TransferEvent,
  ctx: ERC20Context
) {
  try {
    ...
    if (!price) return;
    revenueByToken.add(ctx, amount.times(price), {
      token: await ctx.contract.symbol(),
      type: FEE_TYPE.YT_FEE,
    });
