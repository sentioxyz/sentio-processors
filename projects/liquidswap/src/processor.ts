...
async function getPoolName(coins: [string, string, string]): Promise<string> {
  ...
  return `${coinx.symbol}-${coiny.symbol}-${token}`
...
async function syncPools(ctx: aptos.AptosContext) {
  ...
  const resources = await client.getAccountResources('0x5a97986a9d031c4567e15b797be516910cfcb4156312482efc6a19c0a30c948', { ledgerVersion: version})
