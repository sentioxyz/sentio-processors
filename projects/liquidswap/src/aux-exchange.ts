...
amm.bind()
    ...
    .onEventAddLiquidityEvent(async (evt, ctx) => {
      if (recordAccount) {
        ...
        ctx.logger.info("add liquidity for " + pair, { symbol: coinXInfo.symbol, user: ctx.transaction.sender, value: value, amount: evt.data_decoded.x_added_au, pair: pair, coin: evt.data_decoded.x_coin_type })
        ctx.logger.info("add liquidity for " + pair, { symbol: coinYInfo.symbol, user: ctx.transaction.sender, value: value, amount: evt.data_decoded.y_added_au, pair: pair, coin: evt.data_decoded.y_coin_type })
      }
