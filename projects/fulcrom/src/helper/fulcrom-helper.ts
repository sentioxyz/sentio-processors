...
import { FlpManagerContext } from "../types/eth/flpmanager.js"

let coinInfoMap = new Map<string, Promise<token.TokenInfo>>()

export async function buildTokenInfo(ctx: VaultContext | FlpManagerContext, tokenAddress: string): Promise<token.TokenInfo> {
...
export const getOrCreateToken = async function (ctx: VaultContext | FlpManagerContext, tokenAddress: string): Promise<token.TokenInfo> {
