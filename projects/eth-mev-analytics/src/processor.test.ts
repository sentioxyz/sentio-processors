...
describe("Test MEV", () => {
  ...
  test("2 bots", async () => {
    ...
    assert.ok(ret[0]).toBe(true);
    assert.ok(ret[1].get("0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2")).toBe(
...
*/
  test("sandwich", async () => {
    ...
    assert.ok(mevResults.sandwichTxns).toHaveLength(2);
    assert.ok(mevResults.arbTxns).toHaveLength(2);
  });
  ...
  test("sandwich jared", async () => {
    ...
    const formattedBlock = formatRichBlock(block);
    const mevResults = handleBlock(formattedBlock, chainConfigs[0]);
    assert.ok(mevResults.sandwichTxns).toHaveLength(1);
    assert.ok(mevResults.arbTxns).toHaveLength(2);
  });
  ...
  test("sandwich wrong", async () => {
    ...
    const mevResults = handleBlock(formattedBlock, chainConfigs[0]);
    assert.ok(mevResults.sandwichTxns).toHaveLength(1);
    assert.ok(mevResults.sandwichTxns[0].frontTxnHash).toBe(
  ...
  test("losing sandwich and different decimals", async () => {
    ...
    const formattedBlock = formatRichBlock(block);
    const mevResults = handleBlock(formattedBlock, chainConfigs[0]);
    assert.ok(mevResults.sandwichTxns).toHaveLength(3);
    assert.ok(mevResults.sandwichTxns[0].frontTxnHash).toBe(
  ...
  test("too many uniswap sandwich", async () => {
    ...
    const formattedBlock = formatRichBlock(block);
    const mevResults = handleBlock(formattedBlock, chainConfigs[0]);
    assert.ok(mevResults.sandwichTxns).toHaveLength(0);
  });
  ...
  test("wrong renevue 3", async () => {
    ...
    assert.ok(ret[1].get("0x4e3fbd56cd56c3e72c1403e103b45db9da5b9d2b")).toBe(
      12618978090578852n
    );
  });
  ...
  test("lido shouldn't count", async () => {
    ...
    assert.ok(ret[1].get("0xae7ab96520de3a18e5e111b5eaab095312d7fe84")).toBe(0n);
  });
  ...
  test("huge graph", async () => {
    ...
    assert.ok(ret[0]).toBe(true);
    assert.ok(ret[1].get("0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48")).toBe(
      1039187993n
    );
  });
