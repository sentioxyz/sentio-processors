...
import { dynamic_field, object_ } from "@sentio/sdk/sui/builtin/0x2";
...
SuiWrappedObjectProcessor
  ...
  .onTimeInterval(async (dynamicFieldObjects, ctx) => {
    const nodeType: TypeDescriptor<dynamic_field.Field<object_.ID, linked_table.Node<object_.ID, position.PositionInfo>>>
      = dynamic_field.Field.type(object_.ID.type(), linked_table.Node.type(ANY_TYPE, ANY_TYPE))
