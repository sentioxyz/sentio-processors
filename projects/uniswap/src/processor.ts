...
let poolInfoMap = new Map<string, poolInfo>()
...
for (let i=0;i<poolWatching.length;i++) {
    ...
    UniswapProcessor.bind({address: address}).onEventSwap(
        async function (event: SwapEvent, ctx: UniswapContext) {
            ...
            let token1 : any
            let fee : any
            let localAddress = ctx.address
            console.log("poolInfoMap for " + localAddress + " is " + poolInfoMap.has(localAddress))
            if (!poolInfoMap.has(localAddress)) {
                token0 = await ctx.contract.token0()
                token1 = await ctx.contract.token1()
                fee = await ctx.contract.fee()
                poolInfoMap.set(localAddress, {token0: token0.toString(),
                    token1: token1.toString(), fee: fee.toString()})
                console.log("set poolInfoMap for " + localAddress)
            } else {
                token0 = poolInfoMap.get(localAddress)?.token0
                token1 = poolInfoMap.get(localAddress)?.token1
                fee = poolInfoMap.get(localAddress)?.fee
            }
