...
import { chain, conversion, token } from "@sentio/sdk/lib/utils"
...
const anyEthTotalSupplyProcessor = async function (_: any, ctx: AnyswapERC20Context) {
  const totalSupply = await token.getER20NormalizedAmount(anyEthAddress, await ctx.contract.totalSupply(), 1)
...
const wethBalanceProcessor = async function (block: any, ctx: ERC20Context) {
  const balance = await token.getER20NormalizedAmount(wethAddress, await ctx.contract.balanceOf(anyEthAddress), 1)
...
const handleSwapIn = async function (event: LogAnySwapInEvent, ctx: AnyswapRouterContext) {
  const inAmount = await token.getER20NormalizedAmount(anyEthAddress, event.args.amount, 1)
...
const handleSwapOut1 = async function (event: LogAnySwapOut_address_address_address_uint256_uint256_uint256_Event, ctx: AnyswapRouterContext) {
  const outAmount = await token.getER20NormalizedAmount(anyEthAddress, event.args.amount, 1)
