...
import { BigDecimal, getChainName } from '@sentio/sdk';
...
var totalSupply = BigDecimal(0)
const anyEthTotalSupplyProcessor = async function (_: any, ctx: AnyswapERC20Context) {
  totalSupply = (await ctx.contract.totalSupply()).scaleDown(18)
...
const wethBalanceProcessor = async function (block: any, ctx: ERC20Context) {
  const balance = (await ctx.contract.balanceOf(anyEthAddress)).scaleDown(18)
  ctx.meter.Gauge('weth_balance').record(balance)
  ctx.meter.Gauge('netBalance_old').record(balance.minus(totalSupply))
}
...
const handleSwapIn = async function (event: LogAnySwapInEvent, ctx: AnyswapRouterContext) {
  const inAmount = event.args.amount.scaleDown(18)
  ctx.meter.Gauge('anyswapIn').record(inAmount, { "from": getChainName(event.args.fromChainID.toString()) })
  ctx.meter.Counter('anyswapIn_cume').add(inAmount, { "from": getChainName(event.args.fromChainID.toString()) })
...
const handleSwapOut1 = async function (event: LogAnySwapOut_address_address_address_uint256_uint256_uint256_Event, ctx: AnyswapRouterContext) {
  const outAmount =event.args.amount.scaleDown(18)
  ctx.meter.Gauge('anyswapOut').record(outAmount, { "to": getChainName(event.args.toChainID.toString()) })
  ctx.meter.Counter('anyswapOut_cume').add(outAmount, { "to": getChainName(event.args.toChainID.toString())})
...
const outFilter1 = AnyswapRouterProcessor.filters.LogAnySwapOut_address_address_address_uint256_uint256_uint256(anyEthAddress)
const outFilter2 = AnyswapRouterProcessor.filters.LogAnySwapOut_address_address_string_uint256_uint256_uint256(anyEthAddress)
...
AnyswapERC20Processor.bind({address: anyEthAddress, startBlock: 14215865})
  .onBlockInterval(anyEthTotalSupplyProcessor)

ERC20Processor.bind({address: wethAddress, startBlock: startBlock})
  .onBlockInterval(wethBalanceProcessor)
