    ...
    if (profileInfo) {
      ...
      if (profileInfo) {
        if (profileInfo?.walletAddress) {
          ctx.eventLogger.emit("userProfileInfo", {
            id: profileInfo.id,
            ...
            insHandle: profileInfo.insHandle
          })
        }
      }
    }
...
export async function fetchResults(subject: string) {
  ...
  return await fetch(url.href, {
    ...
    body: `{
      "sqlQuery": {
        "sql": "SELECT id, username, nickname, walletAddress, avatar, userType, fromHoobe, insHandle FROM userProfileInfo WHERE username = '${subject}' ORDER BY timestamp DESC"
      }
    }`,
  })
}
...
async function fetchX(ctx: TomoContext, subject: string): Promise<profileInfo | null> {
  ...
  if (!res.ok) {
    ...
    const nickname = data.basic.nickname
    const walletAddress = data.basic.walletAddress
    const avatar = data.basic.avatar
    const userType = data.basic.userType
    const fromHoobe = data.basic.fromHoobe
    const insHandle = data.basic.insHandle ? data.basic.insHandle : "unk"
