...
import { CreateLongNShortPositionEvent, DeferLiquidityCheckAdapterContext, DeferLiquidityCheckAdapterProcessor, FlashLoanAndLiquidateCallTrace, SwapAndDepositCallTrace, SwapAndRepayCallTrace } from './types/eth/deferliquiditycheckadapter.js'
...
const SwapAndRepayCallHandler = async (call: SwapAndRepayCallTrace, ctx: DeferLiquidityCheckAdapterContext) => {
  if (call.error) {
    return
  }
  console.log("snr account ", call.args.account)
  console.log("tTokenAmount ", call.args.tTokenAmount)
  ...
  const collateralDecimal = (constant.COLLATERAL_DECIMAL.get(collateralSymbol))!
  const repayAmount = scaleDown(call.args.repayAmount, collateralDecimal)
  if (repayAmount.comparedTo(10 ** 30) > 0)
    console.log("tx ", ctx.transactionHash, "collateralDecimal ", collateralDecimal, "repayAmount scaled down", call.args.repayAmount)
...
const SwapAndDepositCallHandler = async (call: SwapAndDepositCallTrace, ctx: DeferLiquidityCheckAdapterContext) => {
  if (call.error) {
    return
  }
  console.log("snd account ", call.args.account)
  const path = call.args.path
  const collateral_address = path[path.length - 1].toLowerCase()
  let collateralSymbol = "unk"
  for (const [key, value] of constant.COLLATERAL_ADDRESSES.entries()) {
...
const CreateLongNShortPositionEventHandler = async (event: CreateLongNShortPositionEvent, ctx: DeferLiquidityCheckAdapterContext) => {
  ...
  //to check
  ctx.meter.Counter("repay_with_collateral_amount").add(Number(event.args.finalLongAmount) / 10 ** collateralDecimal, { collateralSymbol })
}
...
DeferLiquidityCheckAdapterProcessor.bind({
  ...
  .onCallSwapAndDeposit(SwapAndDepositCallHandler)
  .onCallSwapAndRepay(SwapAndRepayCallHandler)
