...
const poolInfos: poolInfo[] = [
  {
    weight: [BigDecimal(80), BigDecimal(20)],
    reserves: [BigDecimal(0), BigDecimal(0)],
    fee: BigDecimal(0.01),
  },
  {
    weight: [BigDecimal(50), BigDecimal(50)],
    reserves: [BigDecimal(0), BigDecimal(0)],
...
async function getTokenInfo(
  ...
  if (address !== "0x0000000000000000000000000000000000000000") {
    try {
      return await token.getERC20TokenInfo(EthChainId.POLYGON, address);
    } catch (e) {
      console.log(e);
      return undefined;
    }
...
async function getPriceByTokenInfo(
  amount: bigint,
  addr: string,
  ctx: VaultContext
): Promise<BigDecimal> {
  ...
  let price: any;
  try {
    price = await getPriceByType(EthChainId.POLYGON, addr, ctx.timestamp);
  } catch (e) {
    console.log(e);
    return BigDecimal(0);
  }
...
async function calOutAmountWeighted(
  amount: BigDecimal,
  weightIn: BigDecimal,
  weightOut: BigDecimal,
  reserveIn: BigDecimal,
  reserveOut: BigDecimal,
  fee: BigDecimal
): Promise<BigDecimal> {
...
async function calInAmountWeighted(
  amount: BigDecimal,
  weightIn: BigDecimal,
  weightOut: BigDecimal,
  reserveIn: BigDecimal,
  ...
  return reserveIn
    .times(power.minus(BigDecimal(1)))
    .dividedBy(BigDecimal(1).minus(fee));
}
...
VaultProcessor.bind({
  ...
  try {
    let [tokens, reserves, lastChangeBlock] = await ctx.contract.getPoolTokens(
      evt.args.poolId,
      { blockTag: ctx.blockNumber, chainId: EthChainId.POLYGON }
    );
    let symbol = "";
    let reserve = "";
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      reserve =
        ...
        token.symbol +
        ": " +
        Number(reserves[i].scaleDown(token.decimal)) +
        ", ";
    }
    ...
    let outAmount = await calOutAmountWeighted(
      evt.args.amountIn.scaleDown(decimalIn),
      weightIn,
      weightOut,
      reserveIn,
      reserveOut,
    ...
    let inAmount = await calInAmountWeighted(
      evt.args.amountOut.scaleDown(decimalOut),
      weightIn,
      weightOut,
      reserveIn,
      reserveOut,
