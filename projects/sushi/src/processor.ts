...
import {
  BentoBoxV1Processor,
  BentoBoxV1Context,
} from "./types/eth/bentoboxv1.js";
import { getPriceByType, token } from "@sentio/sdk/utils";
import { EthChainId } from "@sentio/sdk/eth";
import {
  MasterDeployer,
  MasterDeployerProcessor,
} from "./types/eth/masterdeployer.js";
...
CONTRACT_MAP.forEach((addr, name) => {
  SushiPairProcessor.bind({ address: addr, name: name }).onEventSwap(
    async (evt, ctx) => {
      ...
      if (amount0In > BigInt(0)) {
        price = await getPriceByTokenInfo(
          amount0In,
          token0,
          token0Info,
          ctx.chainId,
...
BentoBoxV1Processor.bind({
  ...
  .onEventLogFlashLoan(async (evt, ctx) => {
    ...
    const price = await getPriceByTokenInfo(
      amount,
      evt.args.token,
      tokenInfo,
      ctx.chainId,
