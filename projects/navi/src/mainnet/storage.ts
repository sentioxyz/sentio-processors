...
const reserves = [
  ...
  "0x2abb6f2b007fef1e59133b027f53eca568f3af79e310e6f16d4b37bc09664a50", // Reserve For SUIUSDT
    ...
    SuiObjectProcessor.bind({
      ...
      async (self, _, ctx) => {
        try {
          ...
          //@ts-ignore
          const ltv = BigDecimal(self.fields.value.fields.ltv).div(
          ...
          const totalSupply = BigDecimal(
            self.fields.value.fields.supply_balance.fields.total_supply
          ).div(Math.pow(10, DEFAULT_COIN_DECIMAL));
          //@ts-ignore
          const totalBorrow = BigDecimal(
            self.fields.value.fields.borrow_balance.fields.total_supply
          ).div(Math.pow(10, DEFAULT_COIN_DECIMAL));

          //@ts-ignore
          const currentSupplyIndex = BigDecimal(
            self.fields.value.fields.current_supply_index
          ).div(Math.pow(10, DECIMAL_RAY));
          //@ts-ignore
          const currentBorrowIndex = BigDecimal(
            self.fields.value.fields.current_borrow_index
          ).div(Math.pow(10, DECIMAL_RAY));
          //add
          //@ts-ignore
          const supplyCapCelling = BigDecimal(
            self.fields.value.fields.supply_cap_ceiling
          ).div(Math.pow(10, DECIMAL_RAY));
          //@ts-ignore
          const borrowCapCeiling = BigDecimal(
            self.fields.value.fields.borrow_cap_ceiling
          ).div(Math.pow(10, DECIMAL_RAY));
          //@ts-ignore
          const treasuryBalance = BigDecimal(
            self.fields.value.fields.treasury_balance
          ).div(Math.pow(10, DEFAULT_COIN_DECIMAL));
          //@ts-ignore
          const currentBorrowRate = BigDecimal(
            self.fields.value.fields.current_borrow_rate
          ).div(Math.pow(10, DECIMAL_RAY));
          //@ts-ignore
          const currentSupplyRate = BigDecimal(
            self.fields.value.fields.current_supply_rate
          ).div(Math.pow(10, DECIMAL_RAY));
          ...
          ctx.meter
            .Gauge("currentSupplyIndex")
            .record(currentSupplyIndex, {
              env: "mainnet",
              id,
              type,
              coin_symbol,
          ...
          ctx.meter
            .Gauge("currentBorrowIndex")
            .record(currentBorrowIndex, {
              env: "mainnet",
              id,
              type,
              coin_symbol,
          ...
          ctx.meter
            .Gauge("supplyCapCeiling")
            .record(supplyCapCelling, {
              env: "mainnet",
              id,
              type,
              coin_symbol,
          ...
          ctx.meter
            .Gauge("borrowCapCeiling")
            .record(borrowCapCeiling, {
              env: "mainnet",
              id,
              type,
              coin_symbol,
          ...
          // borrow rate
          ctx.meter
            .Gauge("currentBorrowRate")
            .record(currentBorrowRate, {
              env: "mainnet",
              id,
              type,
              coin_symbol,
          ...
          ctx.meter
            .Gauge("currentSupplyRate")
            .record(currentSupplyRate, {
              env: "mainnet",
              id,
              type,
              coin_symbol,
