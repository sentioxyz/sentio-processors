...
import { EntityClass, ID, ListFilter, Operators } from "@sentio/sdk/store";
...
export async function storeListAll<T extends EntityWithNetwork, P extends keyof T, O extends Operators<T[P]>>(
  ctx: EthContext,
...
export async function storeDelete<T extends EntityWithNetwork>(
  ...
  const realID = fixEntityID(ctx, id);
  await ctx.store.delete(entity, realID);
}
