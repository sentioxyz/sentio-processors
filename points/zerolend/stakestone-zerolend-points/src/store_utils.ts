...
import {
  ...
  Operators,
  AbstractEntity,
} from "@sentio/sdk/store";
interface EntityWithNetwork extends AbstractEntity {
  network: string;
}

export async function storeListAll<
  T extends EntityWithNetwork,
  P extends keyof T,
  O extends Operators<T[P]>,
>(ctx: EthContext, entity: EntityClass<T>, filters?: ListFilter<T, P, O>[]) {
  return (
...
export async function storeDelete<T extends EntityWithNetwork>(
  ctx: EthContext,
  entity: EntityClass<T>,
  id: string | string[]
) {
  const realID = Array.isArray(id)
    ? id.map((i) => fixEntityID(ctx, i))
    : fixEntityID(ctx, id);
  return ctx.store.delete(entity, realID);
}
