...
import {
  CErc20UpgradableContext,
  CErc20UpgradableProcessor,
} from "./types/eth/cerc20upgradable.js";
...
configs.forEach((config) =>
  CErc20UpgradableProcessor.bind({
...
async function updateAccounts(
  ctx: CErc20UpgradableContext,
  events: TempEvent[]
) {
  const conf = getConfig(ctx.chainId)!;
  const rnpStoneContract = conf.rnpTokenAddress
  ...
  for (const event of events) {
    ...
    promises.push(
      ...
      ctx.contract
        .totalBorrows(overrides)
        .then((v) => [`totalBorrows_${bn}`, v]),
      ctx.contract
        .exchangeRateStored(overrides)
        .then((v) => [`exchangeRateStored_${bn}`, v])
    );
    for (const account of eventAccounts) {
      promises.push(
        ...
        ctx.contract
          .borrowBalanceStored(account, overrides)
          .then((v) => [`borrowBalanceStored_${account}_${bn}`, v]),
        rnpStoneContract &&
  ...
  for (let i = 0; i < eventsWithSentry.length; i++) {
    ...
    if (shouldUpdatePoints) {
      const totalStoneSupply =
        (state.totalSupply * state.exchangeRateRaw) /
        10n ** BigInt(EXCHANGE_RATE_DECIMALS);
      const m = (totalStoneSupply - state.totalBorrow)
    ...
    state.exchangeRateRaw = callResults[`exchangeRateStored_${bn}`];
    for (const account of eventAccounts) {
      ...
      const sbStoneBalance = callResults[`balanceOf_${account}_${bn}`];
      const borrowBalance = callResults[`borrowBalanceStored_${account}_${bn}`];
      const newRnpStoneBalance =
        callResults[`rnpStone_balanceOf_${account}_${bn}`];
      const stoneBalance =
        ((sbStoneBalance + newRnpStoneBalance) * state.exchangeRateRaw) /
        10n ** BigInt(EXCHANGE_RATE_DECIMALS);
