...
import { ADDRESSS, DAILY_POINTS, MULTIPLIER, NETWROK } from "./config.js";
...
function calcPoints(ctx: ERC20Context, snapshot: AccountSnapshot): BigDecimal {
...
async function getAccountSnapshot(ctx: ERC20Context, account: string) {
  return new AccountSnapshot({
    ...
    lbtcBalance: await ctx.contract.balanceOf(account),
  });
}
