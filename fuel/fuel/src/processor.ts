...
import {
  FuelERC20GatewayV4Context,
  FuelERC20GatewayV4Processor,
} from "./types/eth/fuelerc20gatewayv4.js";
import { getPriceByType, token } from "@sentio/sdk/utils";
import { BigDecimal, scaleDown } from "@sentio/sdk";

const startBlock = 20990000;
...
async function getUsdValue(
  ...
  const price =
    (await getPriceByType(EthChainId.ETHEREUM, token, ctx.timestamp)) || 0;
...
async function recordBalance(
  ...
  const usd = await getUsdValue(
    ctx,
    tokenAddress,
    amount.scaleDown(tokenInfo.decimal),
  );
