...
async function handleEvent(evt: pool.AddLiquidityEventInstance, ctx: SuiContext) {
  ...
  if (symbol_a.includes('USD') || symbol_b.includes('USD')) {
    ...
    recordTx(ctx, evt.sender, symbol, 'cetus')
  }
}
...
pool
  ...
  .onEventSwapEvent(async (evt, ctx) => {
    ...
    if (symbol_a.includes('USD') && symbol_b.includes('USD')) {
      ...
      recordSwap(ctx, evt.sender, pair, decimal_a, decimal_b, amount_in, amount_out, 'cetus')
    }
  })
...
//   console.log('new cetus pool', pool_id, coin_type_a, coin_type_b)
