...
SuiWrappedObjectProcessor.bind({
  ...
  .onTimeInterval(async (dynamicFieldObjects, ctx) => {
    try {
      ...
      const fields = await ctx.coder.getDynamicFields(dynamicFieldObjects, string$.String.type(), objectType)
...
for (let i = 0; i < constant.POOLS_MOLE_LIST.length; i++) {
  SuiObjectProcessor.bind({
    ...
    .onTimeInterval(async (self, _, ctx) => {
      try {
        const res = await ctx.coder.decodeType(self, pool.Pool.type())
        //@ts-ignore
    ...
    .onTimeInterval(async (self, _, ctx) => {
      ...
      try {
        ...
        if (workerInfoAddr == "0x98f354c9e166862f079aaadd5e85940c55c440a8461e8e468513e2a86106042c") {
          ...
          res = await ctx.coder.decodeType(self, cetus_clmm_worker_weth_usdc.WorkerInfo.type())
        } else if (workerInfoAddr == "0x1774ca4f9e37f37c6b0df9c7f9526adc67113532eb4eaa07f36942092c8e5f51") {
          res = await ctx.coder.decodeType(self, cetus_clmm_worker_usdc_weth.WorkerInfo.type())
