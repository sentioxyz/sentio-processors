...
pool.bind({
  ...
  .onEventSwapEvent(async (event, ctx) => {
    ...
    ctx.eventLogger.emit("trading_vol_gauge", {
      usd_volume,
      pairName: poolInfo.pairName, 
      project: "cetus" 
    })
  ...
  .onEventAddLiquidityEvent(async (event, ctx) => {
    ...
    ctx.eventLogger.emit("add_liquidity_gauge", {
      value, 
      pairName 
    })
  ...
  .onEventRemoveLiquidityEvent(async (event, ctx) => {
    ...
    ctx.eventLogger.emit("remove_liquidity_gauge", {
      value,  pairName 
    })
  })
  ...
  .onTimeInterval(async (self, _, ctx) => {
    ...
    try {
      ...
      if (coin_a_balance) {
        ...
        ctx.eventLogger.emit("coin_a_balance", {
          coin_a_balance,
      ...
      if (coin_b_balance) {
        ...
        ctx.eventLogger.emit("coin_b_balance", {
          coin_b_balance,
      ...
      ctx.eventLogger.emit("a2b_price", {
        coin_a2b_price,
        pairName, 
        project: "cetus" 
      })
      ...
      ctx.eventLogger.emit("b2a_price", {
        coin_b2a_price,
        pairName, 
        project: "cetus" 
      })
