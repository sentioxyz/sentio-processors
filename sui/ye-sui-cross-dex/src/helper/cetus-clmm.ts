...
export async function buildPoolInfo(ctx: SuiContext | SuiObjectContext, pool: string): Promise<poolInfo> {
    ...
    try {
        ...
        if (fields?.fee_rate) {
            fee_label = (Number(fields.fee_rate) / 10000).toFixed(2) + "%"
        }
        else {
...
export async function getPoolPrice(ctx: SuiContext | SuiObjectContext, pool: string) {
    ...
    ctx.meter.Gauge("a2b_price").record(coin_a2b_price, { pairName, pairFullName, project: "cetus" })
    ctx.meter.Gauge("b2a_price").record(coin_b2a_price, { pairName, pairFullName, project: "cetus" })
